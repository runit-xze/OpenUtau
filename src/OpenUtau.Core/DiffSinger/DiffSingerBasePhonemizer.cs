...
namespace OpenUtau.Core.DiffSinger {
	public abstract class DiffSingerBasePhonemizer : MachineLearningPhonemizer {
		...
		private bool _singer_loaded;

		public override void SetSinger(USinger singer) {
			if (_singer_loaded && singer == this.singer) return;
			try {
				_singer_loaded = _execute_set_singer(singer);
			} catch {
				_singer_loaded = false;
				throw;
			}
		}

		private bool _execute_set_singer(USinger singer) {
			this.singer = singer;
			if (singer == null) {
