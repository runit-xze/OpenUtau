	...
	public class ArpasingPlusPhonemizer : SyllableBasedPhonemizer {
			...
			for (var i = firstC; i < lastC; i++) {
				...
				if (!HasOto(cc1, syllable.tone) || (!HasOto(ValidateAlias(cc1), syllable.tone) && !HasOto($"{cc[i]} {cc[i + 1]}", syllable.tone))) {
					...
					if (!c1IsException && !c2IsException) {
						cc1 = AliasFormat($"{c2}", "cc_in_b", syllable.vowelTone, "");
						TryAddPhoneme(phonemes, syllable.tone, ValidateAlias(AliasFormat($"{c1}", "cc_end_b", syllable.vowelTone, "")));
					}
					...
					// Scenario 3: C1 is NOT an exception, C2 is
						...
						cc1 = AliasFormat($"{c1}", "cc_end_b", syllable.vowelTone, "");
					}
				...
				for (var i = firstC; i < lastC; i++) {
					...
					if (i < cc.Length - 2) {
						...
						if (!HasOto(cc2, ending.tone) && !HasOto($"{cc[i + 1]} {cc[i + 2]}", ending.tone)) {
							// [C1 -] [- C2]
							cc2 = AliasFormat($"{cc[i + 2]}", "cc_in_b", ending.tone, "");
							TryAddPhoneme(phonemes, ending.tone, ValidateAlias(AliasFormat($"{cc[i + 1]}", "cc_end_b", ending.tone, "")));
						}
						if (!HasOto(cc1, ending.tone)) {
							cc1 = ValidateAlias(cc1);
						}
						...
						if (HasOto(cc1, ending.tone) && (HasOto(cc2, ending.tone) || HasOto($"{cc[i + 1]} {cc[i + 2]}{t}", ending.tone) || HasOto(ValidateAlias($"{cc[i + 1]} {cc[i + 2]}{t}"), ending.tone))) {
							...
							// [C1 -] [- C2]
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[i + 1]}", "cc_in_b", ending.tone, ""));
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[i + 1]}", "cc_end_b", ending.tone, ""));
							i++;
						} else {
							// like [C1][C2 ...]
						...
						if (!HasOto(cc1, ending.tone) || !HasOto(ValidateAlias(cc1), ending.tone) && !HasOto($"{cc[i]} {cc[i + 1]}", ending.tone)) {
							cc1 = AliasFormat($"{cc[i + 1]}", "cc_in_b", ending.tone, "");
							TryAddPhoneme(phonemes, ending.tone, ValidateAlias(AliasFormat($"{cc[i]}", "cc_end_b", ending.tone, "")));
						}
						if (!HasOto(cc1, ending.tone)) {
							cc1 = ValidateAlias(cc1);
						}
						...
						if (!phoneticHint && (TryAddPhoneme(phonemes, ending.tone, $"{cc[i]} {cc[i + 1]}{t}", ValidateAlias($"{cc[i]} {cc[i + 1]}{t}")))) {
							...
							// [C1 -] [- C2]
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[i + 1]}", "cc_in_b", ending.tone, ""));
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[i + 2]}", "cc_end_b", ending.tone, ""));
							i++;
						}
					}
				}
		...
		private string AliasFormat(string alias, string type, int tone, string prevV, string t = "-") {
            ...
            // Define alias formats for different types
				...
				{ "cc_in_b", new string[] { "_", "-", "- " } },
				{ "cc_end_b", new string[] { "_", $"{t}", $" {t}" } },
