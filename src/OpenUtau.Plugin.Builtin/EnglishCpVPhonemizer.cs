...
namespace OpenUtau.Plugin.Builtin {
	...
	public class EnglishCpVPhonemizer : SyllableBasedPhonemizer {
		...
		protected override byte[] YamlTemplate => Data.Resources.en_c_pv_template;
		...
		protected override List<string> ProcessEnding(Ending ending) {
			...
			if (ending.IsEndingV) {
				...
				if (!RomajiException.Contains(cc[0])) {
					if (HasOto(vcr, ending.tone) || HasOto(ValidateAlias(vcr), ending.tone)) {
					...
					} else if (DiphthongExceptions.ContainsKey(prevV) && (c_c_r.Contains(cc.Last())) && ((HasOto(AliasFormat(v, "ending_mix", ending.tone, ""), ending.tone) && (HasOto($"{c_c_r[0]} {t}", ending.tone) || (HasOto($"{c_c_r[0]}{t}", ending.tone)))))) {
						// ex: [ow][ow-][z][z -]
						...
						TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{DiphthongExceptions[prevV]}", "diph_mix", ending.tone, "", t));
						if (c_c_r.Contains(cc.Last())) {
							if (HasOto(AliasFormat($"{c_c_r[0]}", "cc_mix", ending.tone, ""), ending.tone)) {
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc1_mix", ending.tone, "", t));
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc_mix", ending.tone, "", t));
							} else if (!(HasOto(AliasFormat($"{c_c_r[0]}", "cc_mix", ending.tone, ""), ending.tone))) {
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc1_mix", ending.tone, "", t));
							} else {
								TryAddPhoneme(phonemes, ending.tone, $"{cc[0]} {t}", $"{cc[0]}{t}");
							}
						} else if (!c_c_r.Contains(cc.Last())) {
							if (HasOto(AliasFormat($"{c_c_r[0]}", "cc_mix", ending.tone, ""), ending.tone)) {
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc_mix", ending.tone, "", t));
							} else if (!(HasOto(AliasFormat($"{c_c_r[0]}", "cc_mix", ending.tone, ""), ending.tone))) {
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc1_mix", ending.tone, "", t));
						...
						if (HasOto(vc, ending.tone) || HasOto(ValidateAlias(vc), ending.tone)) {
							...
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc_mix", ending.tone, "", t));
						} else if (HasOto($"{c_c_r[0]} {t}", ending.tone) || HasOto(ValidateAlias($"{c_c_r[0]} {t}"), ending.tone) || (HasOto($"{c_c_r[0]}{t}", ending.tone) || HasOto(ValidateAlias($"{c_c_r[0]}{t}"), ending.tone))) {
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc1_mix", ending.tone, "", t));
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc_mix", ending.tone, "", t));
						} else if (!(HasOto($"{c_c_r[0]} {t}", ending.tone) || HasOto(ValidateAlias($"{c_c_r[0]} {t}"), ending.tone) || (HasOto($"{c_c_r[0]}{t}", ending.tone) || HasOto(ValidateAlias($"{c_c_r[0]}{t}"), ending.tone)))) {
							TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[0]}", "cc_mix", ending.tone, "", t));
				...
				for (var i = firstC; i < lastC; i++) {
					...
					if (i < cc.Length - 2) {
						...
						if (TryAddPhoneme(phonemes, ending.tone, $"{cc[i]} {cc[i + 1]}{t}", ValidateAlias($"{cc[i]} {cc[i + 1]}{t}"))) {
							...
							if (HasOto($"{c_c_r[0]} {t}", ending.tone) || HasOto(ValidateAlias($"{c_c_r[0]} {t}"), ending.tone) || (HasOto($"{c_c_r[0]}{t}", ending.tone) || HasOto(ValidateAlias($"{c_c_r[0]}{t}"), ending.tone))) {
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[i]}", "cc1_mix", ending.tone, "", t));
								TryAddPhoneme(phonemes, ending.tone, AliasFormat($"{cc[i + 1]}", "cc1_mix", ending.tone, "", t));
