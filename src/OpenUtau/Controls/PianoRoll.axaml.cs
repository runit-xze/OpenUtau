...
namespace OpenUtau.App.Controls {
	...
	public partial class PianoRoll : UserControl, IValueTip, ICmdSubscriber {
		...
		private CancellationTokenSource? _long_press_cts;
		private async void OnToolButtonPointerPressed(object? sender, PointerPressedEventArgs args) {
			...
			if (sender is Control control) {
				_long_press_cts = new CancellationTokenSource();
				try {
					await Task.Delay(500, _long_press_cts.Token);
					if (_long_press_cts != null && !_long_press_cts.IsCancellationRequested) {
		...
		private void OnToolButtonPointerReleased(object? sender, PointerReleasedEventArgs args) {
			_long_press_cts?.Cancel();
			_long_press_cts?.Dispose();
			_long_press_cts = null;
		}
